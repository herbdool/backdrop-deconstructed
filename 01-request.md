...
Have you ever wondered how Backdrop does what it does? Good, me too. In this series of posts, I'm going to explain what Backdrop is doing behind the scenes to perform its magic.

In Part 1, we'll keep it fairly high level and walk through the path a request takes as it moves through Backdrop. In later parts, we'll take deeper dives into individual pieces of this process.
...
2. Allows requests that point directly to files in the filesystem to pass through without touching Backdrop.
3. Redirects all other requests to Backdrop's `index.php` file.

It also does some other fancy things such as disabling directory indexes and allowing Backdrop to serve gzipped CSS and JS, but those are the biggies.
...
## Step 3: Backdrop's index.php file

Finally, we have reached Backdrop, and we're looking at PHP. Backdrop's index.php is super clean, and in fact only has 4 lines of code, each of which are easy to understand.

### Line 1: Define BACKDROP_ROOT

```php
define('BACKDROP_ROOT', getcwd());
```

This just sets a constant called `BACKDROP_ROOT` to be the value of the current directory that `index.php` is in. This constant is [used all over the place](https://docs.backdropcms.org/api/backdrop/index.php/constant/BACKDROP_ROOT/1) in the Backdrop core codebase.

In our case, this means that `BACKDROP_ROOT` gets set to `/var/www/oursite`.

### Lines 2 and 3: Bootstrap Backdrop
