...
Alright! Now we're getting somewhere! Here's what we have so far. We have our request to `/about-us` and we've already run through a full Backdrop bootstrap, thanks to `index.php`.

Now comes the fun part.

## A quick summary

If you want the nickel tour of the Backdrop menu router (or you don't want to look at any code), then you're in the right place. Here's what you can expect.
...
We start with step 0 because it has already happened. Remember in the Bootstrap chapter that, during the `BACKDROP_BOOTSTRAP_FULL` phase, the [`backdrop_path_initialize()`](https://docs.backdropcms.org/api/backdrop/core!includes!path.inc/function/backdrop_path_initialize/1) function is called. That function just makes sure that `$_GET['q']` is set (if it's not, then it sets it to the frontpage URL).

Then, more importantly, it runs through [`backdrop_get_normal_path()`](https://docs.backdropcms.org/api/backdrop/core!includes!path.inc/function/backdrop_get_normal_path/1) to see if we are looking at a path alias rather than an internal path, and if so, replaces it with the internal path that it's aliasing.
...
Our `index.php` file calls the grand [`menu_execute_active_handler()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/menu_execute_active_handler/1) function.

First, it checks to see if the site is offline and bypasses a lot of logic if that's the case. It even gives modules a chance to have their say as to whether the site is offline, using [`hook_menu_site_status_alter()`](https://docs.backdropcms.org/api/backdrop/core!modules!system!system.api.php/function/hook_menu_site_status_alter/1).
...
From here, we call the [`menu_get_item($path)`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/menu_get_item/1) function, which does many things, all of which basically amount to "tell us everything we need to know about what to do with this path".
...
So before we query `menu_router`, we need to call [`menu_get_ancestors($path)`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/menu_get_ancestors/1) to fetch the possible ancestor paths for the given path. Only then can we query `menu_router` to find the path that fits the best. Here's how that all looks at a code level.
...
Before we can continue on, we need to give modules the ability to alter the menu item right away, before anything else happens, using [`hook_menu_get_item_alter()`](https://docs.backdropcms.org/api/backdrop/core!modules!system!system.api.php/function/hook_menu_get_item_alter/1).

```php
backdrop_alter('menu_get_item', $router_item, $path, $original_map);
```

This differs from the more commonly used [`hook_menu_alter()`](https://docs.backdropcms.org/api/backdrop/core!modules!system!system.api.php/function/hook_menu_alter/1) in that this is at run-time, as opposed to `hook_menu_alter()` which only runs when the menu system is being built, and doesn't run as part of a page request.
...
It's confusing first of all because it happens in a function called [`_menu_translate()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_translate/1) which means "translate" as in "translate placeholders in the URL to loaded entities (or other things), when needed". (It also translates the menu title to the appropriate language later in this same function, confusing things further).
...
To do this, it runs [`_menu_load_objects()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_load_objects/1) which calls the appropriate load function if there is one and, assuming we don't get an access error, a missing entity, or anything else which would cause a problem, put the result (i.e., the fully loaded entity) back into the menu router item for later use.
...
That part happens via a call to [`_menu_check_access()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_check_access/1), which basically runs the function set in `access_callback` if one exists, otherwise it falls back to [`user_access()`](https://docs.backdropcms.org/api/backdrop/core!modules!user!user.module/function/user_access/1), and includes any defined `access_arguments`. The fallback to `user_access()` means that we can just pass in a permission as `access_arguments` completely and leave out `access_callback` in our `hook_menu()` and it works out just peachy, which is a nice little shortcut.
...
I mentioned earlier that we'd talk about how the menu router gets assembled in the first place. In other words, what exactly happens in the oh-so-scary [`menu_rebuild()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/menu_rebuild/1) function that takes forever to run?
...
The [`menu_router_build()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/menu_router_build/1) function basically calls all `hook_menu()` implementations and builds a giant array out of them.

This is actually an incredibly scary process. Don't be fooled by how simple `menu_router_build()` looks; the magic really happens in [`_menu_router_build()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_router_build/1) (underscore functions strike again!). Take a look at that one to see what I mean.
...
Finally, an easy one. The [`_menu_router_save()`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_router_save/1) function basically just does a giant `db_insert()` into the menu_router table for each menu item returned by `menu_router_build()`.
...
Since `hook_menu()` lets you place menu items in actual user facing navigation menus using the `MENU_NORMAL_ITEM` type and the optional `menu_name` key, we need to actually do the work of putting those items into navigation menus. This happens in [`_menu_navigation_links_rebuild`](https://docs.backdropcms.org/api/backdrop/core!includes!menu.inc/function/_menu_navigation_links_rebuild/1).
...
That's all well and good, but what would trigger this to happen? How does Backdrop know when it's time to run rebuild the menu router?
...
There are a couple obvious things that trigger menu rebuilds. One of the most common is  [`backdrop_flush_all_caches()`](https://docs.backdropcms.org/api/backdrop/core!includes!common.inc/function/backdrop_flush_all_caches/1) function. Besides being called whenever you manually flush caches (which is about 9000 times a day if you're anything like me), it also gets called by [`system_modules_submit()`](https://docs.backdropcms.org/api/backdrop/core!modules!system!system.admin.inc/function/system_modules_submit/1) when enabling a new module, so that the module's `hook_menu()` or `hook_menu_alter()` implementations (or other menu related hooks) can be respected.

Besides that one, there are a few other places that call `menu_rebuild()` directly. Most notably:

- [`node_type_form_submit()`](https://docs.backdropcms.org/api/backdrop/core!modules!node!content_types.inc/function/node_type_form_submit/1) - for adding tabs and paths for a content type that is being created or updated
- [`node_type_delete_confirm_submit()`](https://docs.backdropcms.org/api/backdrop/core!modules!node!content_types.inc/function/node_type_delete_confirm_submit/1) - for doing the exact opposite: deleting all that stuff.
- [`theme_enable()`](https://docs.backdropcms.org/api/backdrop/core!includes!theme.inc/function/theme_enable/1) and [`theme_disable()`](https://docs.backdropcms.org/api/backdrop/core!includes!theme.inc/function/theme_disable/1) - for adding or removing tabs and paths related to a theme that is being enabled or disabled
...
- [`field_ui_field_attach_create_bundle()`](https://docs.backdropcms.org/api/backdrop/core!modules!field_ui!field_ui.module/function/field_ui_field_attach_create_bundle/1) - if we're adding a new bundle to a fieldable entity type, then we need to add menu item tabs for it
- [`image_system_file_system_settings_submit()`](https://docs.backdropcms.org/api/backdrop/core!modules!image!image.module/function/image_system_file_system_settings_submit/1) - if we're saving the File System configuration form and update the public files path setting, then we need to rebuild the menu to reflect the new path.

Those are the big ones as far as core is concerned. If you search the standard Backdrop site's codebase for `menu_rebuild` then you'll find a bunch more, but most of them belong to either contrib modules or automated test cases. Runtime core code does a relatively good job of only doing a full menu rebuild when it's absolutely necessary.
