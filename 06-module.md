...
The major difference between Backdrop and most other CMSes is its modular
...
which improves Backdrop's performance and reduces its exposure to attacks.

All of the functions that define the module subsystem live in
[module.inc](https://docs.backdropcms.org/api/backdrop/includes!module.inc/1).
You may remember from chapter 2 that this file is first included during the `BACKDROP_BOOTSTRAP_VARIABLES`
...
directory within your Backdrop installation where Backdrop will know to scan
for the code. Prior to this point, all of the functionality of Backdrop we
...
reinstallation) and then invoke hook_modules_installed() and
...
[system_rebuild_module_data()](https://docs.backdropcms.org/api/backdrop/modules!system!system.module/function/system_rebuild_module_data/1) is itself implemented in a module
...
1. Use [backdrop_system_listing()](https://docs.backdropcms.org/api/backdrop/includes!common.inc/function/backdrop_system_listing/1)
...
4. If the directory it is in also has a .info file, call
   backdrop_parse_info_file() on it. That function is basically just a
   cache wrapper around backdrop_parse_info_format(), which is largely
   ...
   .info files (Backdrop)](https://www.drupal.org/node/542202). There
...
7. Invoke hook_system_info_alter(). This is worth a whole subchapter on
   its own, because hooks are how Backdrop does its module magic.
...
hooks, we call [module_implements()](https://docs.backdropcms.org/api/backdrop/includes!module.inc/function/module_implements/1)
...
calls to the `backdrop_alter()` function.

Considering how important it is, `backdrop_alter()` is a surprisingly small
...
`backdrop_alter()` takes two required arguments:
...
Because `backdrop_alter()` is frequently called, it uses the advanced
`backdrop_static()` pattern, as described in the
[performance](18-performance.md) chapter. `backdrop_alter()` may take either
...
[module_load_all](module_load_all) function.
...
implement hook_boot, hook_exit, hook_watchdog, or
hook_language_init). During BACKDROP_BOOTSTRAP_VARIABLES, it is called
with a value of true.

The rest of the modules are not loaded until BACKDROP_BOOTSTRAP_FULL.
...
`module_list()` is a fairly thin wrapper around [system_list()](https://docs.backdropcms.org/api/backdrop/includes!module.inc/function/system_list/1). system_list() can return either:
...
Once we have the list, we call [backdrop_load()](https://docs.backdropcms.org/api/backdrop/includes!bootstrap.inc/function/backdrop_load/1)
on each module in the list. backdrop_load is basically just a cache
...
needed by virtue of either Backdrop core or another module calling the
[module_invoke()](https://docs.backdropcms.org/api/backdrop/includes!module.inc/function/calls/module_invoke/1)
