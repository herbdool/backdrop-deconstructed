...
Ah yes. The form system. Who doesn't love the Backdrop form system, right? Right?

The typical Backdrop developer can probably tell you how to build a form and what the big array of fields should look like, but it's very doubtful that that developer has any idea how Backdrop takes that big array and displays it on the page, or validates it, or decides whether validation passed, or figures out which button was clicked to submit the form, or dozens of other useful things to know.
...
To start, [`backdrop_get_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_get_form/1) is called which runs the form builder function to assemble the form array. Then it gets processed a bit (lots of details, none of which are worth of mention in our "quick summary") and handed off to the render system for display. The render system is discussed in a later chapter so I'll save the details of converting a form array into HTML markup there.

When the form is submitted, the page reloads which means that the exact same thing happens - `backdrop_get_form()` is called which runs the form builder function to assemble the form array. The difference is that this time, the form submission is grabbed from `$_GET` or `$_POST` (Backdrop tries to do that every time, even on viewing, and it just ignores the result if there isn't any valid submission to process). Backdrop then recognizes that there is a valid form submission, and flags it for validation and submission.

For validation, it has a few hardcoded things to check for, such as `#required`, `#maxlength`, and a validation `#value` that exists in the `#options` array for applicable form elements. Then it runs any user-defined validation callbacks for the form. Any one of those can run [`form_set_error()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_set_error/1) to create an error on a form element, which tells Backdrop not to submit the form.

Once validation is done, we check to see if [`form_get_errors()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_get_errors/1) is empty, and if not, we run any user defined submission callbacks for the form.
...
In the beginning, there is (usually) a call to [`backdrop_get_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_get_form/1). This could be the function defined as a page callback in `hook_menu()`, or it could be called from a `hook_block_view()`, or it could be included in the `content_type` plugin for a Panels pane, or any number of other things. But whatever you're talking about, if it involves displaying a form, then you're probably going to be calling `backdrop_get_form()` to fetch it, and you're probably going to be passing in the name of your custom form builder function as the `$form_id` parameter.
...
This is already a little tricky because of the argument handling. The `backdrop_get_form()` function is kind of neat in that it expects the first argument to be `$form_id` (which will typically be the name of the function that builds the form array and returns it), but you can pass in any arbitrary arguments after that first one to provide your form builder function with any data that it might need. You can see that anything you pass in after `$form_id` gets added to `$form_state ['build_info']['args']` so that in your form builder function, you can fetch it out of there.

Anyways, we end up calling [`backdrop_build_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_build_form/1), which is a workhorse of a function. It does a lot of things, so let's take them one step at a time.

### Set some defaults

The first thing that `backdrop_build_form()` does is adds some default values to the `$form_state` array, like so:

```php
$form_state += form_state_defaults();
```

Looking at [`form_state_defaults()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_state_defaults/1) shows us this:
...
The second important function of `backdrop_build_form()` is to try to grab the form from the forms cache, if it exists there.
...
So what happens in [`form_get_cache()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_get_cache/1)? Let's find out.
...
And that's it. At the end of it all, we just return `$form` if it was set back at the beginning. Otherwise, we return nothing. Then, back in [`backdrop_build_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_build_form/1), all we have left to do is process the form using `backdrop_process_form()` (more on that in a bit), and return it.

### If not cached, build the form from scratch

Assuming the cache didn't give us anything to work with, we need to piece it together ourselves. Still inside [`backdrop_build_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_build_form/1), we use two very important functions to do that.
...
The [`backdrop_retrieve_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_retrieve_form/1) is basically a big fancy guy that, at the end of the day, just calls the original form function to populate the `$form` array..
...
- If there's no function with a name matching the value of `$form_id`, then it runs implementations of [`hook_forms`](https://docs.backdropcms.org/api/backdrop/modules%21system%21system.api.php/function/hook_forms/1) to try to find a matching form builder function for that `$form_id`.
...
- `$form['#id']` is set as a CSS friendly version of `$form_id`, by running it through [`backdrop_html_id()`](https://docs.backdropcms.org/api/backdrop/includes%21common.inc/function/backdrop_html_id/1)

#### Register the form validation callbacks (if they exist)

Form validation is handled by validation callbacks. Validation callbacks are both optional and inheritable. The source of truth for "how to validate this form" lives in `$form['#validate']`, which is an array of validation functions. If it is not already set when `backdrop_prepare_form` is called, Backdrop initializes an empty array. It then checks to see if there's a function whose name matches `$form_id . '_validate'`. If so, we add that to the `$form['#validate']` array.
...
So we start with [`hook_form_alter()`](https://docs.backdropcms.org/api/backdrop/modules%21system%21system.api.php/function/hook_form_alter/1), then we add on [`hook_form_BASE_FORM_ID_alter()`](https://docs.backdropcms.org/api/backdrop/modules%21system%21system.api.php/function/hook_form_BASE_FORM_ID_alter/1), and finally we add [`hook_form_FORM_ID_alter()`](https://docs.backdropcms.org/api/backdrop/modules%21system%21system.api.php/function/hook_form_FORM_ID_alter/1), in that order.
...
The [`backdrop_process_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_process_form/1) function is really the heart of the form API. This is where the form gets assembled, validated, and submitted.

For this section, we're only interested in displaying the form, so we'll get to validation and submission later.

#### Build the form elements

Yes, I know this is confusing, because we've already "built" the form by calling its form builder function to grab the form array. What's happening here is different. The `backdrop_process_form()` function calls [`form_builder()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_builder/1). This recursive function cycles through the form tree from top to bottom, and for each element, does the following:
...
The [`_form_builder_handle_input_element()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/_form_builder_handle_input_element/1) function is long (due to the comments) and somewhat scary looking, but its main purposes are assigning a `#name` to nested fields, and assigning a `#value` if one exists in `$form_state['input']`.
...
One interesting thing about the Form API is that the call stack is the same whether we're viewing, validating, or submitting. When the user submits a form, it just reloads the page and  therefore re-runs `backdrop_get_form()`, but the difference this time around is that Backdrop recognizes that the form was just submitted and thus flags it for input processing.

Here's how that happens.

### Flag the form as "submitted"

This process starts early on, in `backdrop_build_form()` (which, as you may remember, gets called from `backdrop_get_form()` which is the function that starts everything):
...
Backdrop basically says "if `$form_state['input']` isn't empty, and the `form_id` in it is the same as the `$form_id` of the form we're currently building, then we know that we have input ready for processing." So the `$form_id` check is the safety against the problem mentioned above.
...
Pretty simple. The only gotcha is that it makes sure that the form wasn't submitted programmatically via the [`backdrop_form_submit()`](https://docs.backdropcms.org/api/backdrop/core%21includes%21form.inc/function/backdrop_form_submit/1) function (which sets `$form_state['programmed']` to `TRUE`), because if it was, then there would have been no button clicked at all.
...
Back in [`backdrop_process_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_process_form/1), it's time to validate the form, if there's a submission to validate.
...
Let's see what actually happens in [`backdrop_validate_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_validate_form/1). Turns out, that function calls [`_form_validate()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/_form_validate/1) which is where the heavy lifting happens.
...
That simple call to [`form_execute_handlers()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_execute_handlers/1) tells Backdrop to run any custom validation callbacks. These are defined as things `$form_state['validate_handlers']` array if populated (this is described in the "Look for validate/submit handlers on the triggering element" section earlier in this chapter), otherwise it checks the `form['#validate']` array.
...
We're back in `backdrop_validate_form()` now, and all validation has had a chance to run at this point.

But before moving onto either submitting the form if it validated or reloading it with errors if not, we have one more step. We need to check the [`#limit_validation_errors`](https://docs.backdropcms.org/api/backdrop/developer!topics!forms_api_reference.html/7#limit_validation_errors) property on the triggering element (i.e., the button used to submit the form).

This property can be set on form buttons to tell Backdrop to ignore any failed validation on elements that aren't specifically listed. This is most commonly used for things like AJAX calls that aren't meant to validate and submit the entire form, such as "Add More" for adding new empty inputs to a form without submitting it.
...
Before we move onto submission, it's worth looking at how errors actually get set. When a validation function calls [`form_set_error()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/form_set_error/1) to flag a validation failure, what happens?
...
Back in [`backdrop_process_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_process_form/1), it's time to submit the form.
...
The [`backdrop_redirect_form()`](https://docs.backdropcms.org/api/backdrop/includes%21form.inc/function/backdrop_redirect_form/1) function is called at this point.
...
Note that this supports passing an array into `$form_state['redirect']` instead of just a string path. Doing so will pass the array pieces into `backdrop_goto()` as arguments, which means that you can pass along options, which end up being passed to the `url()` function.
